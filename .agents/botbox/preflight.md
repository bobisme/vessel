...
1. Resolve agent identity: use `--agent` argument if provided, otherwise `$AGENT` env var. If neither is set, adopt `<project>-dev` (e.g., `botbox-dev`). Agents spawned by `botbox run worker-loop` receive a random name automatically.
